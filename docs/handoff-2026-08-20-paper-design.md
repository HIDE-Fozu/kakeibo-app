...
- **紙テクスチャ・生成り背景は撤回**（「紙っぽくない」FB・32977b8）。背景は従来の
  ウォームアイボリーに戻した。追記2のテクスチャの節は歴史として残すが現物は無い。
- **セル内は日付=左上・金額（実績/予定）=右揃え**。
- **明朝トライアル採用**: kLedgerFontFamily（Hiragino Mincho ProN）を
  カレンダー画面の月見出し・サマリカード・見込み行・曜日・日付数字・セル金額・
  日付タブに適用（本文/ボタン/日別リストはゴシックのまま）。
  他画面への展開は未定（ユーザーの見た目レビュー待ち）。

### 追記4（同日FB第4弾・実機配備）

- **サマリカードの縦幅拡大**（padding14・金額17pt・仕切り40）・**下タブ height62**。
- **支出/収入の意味色を深く**: kExpense D97C6C→B5483A・kIncome 59A98A→3E8465
 （「金額の色が明るすぎる」FB。意味色を使う全箇所に効く）。
- **実機 TI10B1 に profile ビルドを配備・起動済み（HEAD=3dcee6a時点・上書きインストール）**。
  ごみ箱の schema v11 マイグレーションもこのビルドで実機適用される。

### 追記5（同日FB第5弾）

- **見込み収支の基準日切り替えを撤去**（「多分月末までしか表示できてないし、不要」FB）:
  常に月末固定。▾・基準日シート・forecastAnchorDay設定・l10n4キー・テスト1本を削除。
  毎月タブの見込み行も月末ラベル固定。**ストア状況: 公開中のv2.1.0には見込み収支自体が
  未搭載。審査待ちのv2.2.0(46)には基準日切り替え込みで入っている（自動リリース設定）**。
  → 対応方針は次回ユーザーと確認（そのまま通して2.3.0で撤去 or 審査取り下げ）。
- **日別セクションに「つきいち」タブ**: 選択日タブ（取引・予定）とつきいちタブ
  （家事の記録・期日行）に分離。buildChoreDayRows は calendar_screen の
  _ChoreDayList へ移設し、日別リストから家事行を撤去。
- **入力ボタンは「＋ 家計簿を入力」**（ラベル付き・homeFabEntryLabel文言変更×9・
  キーfab-entry維持）。タブ行は「ボタン固定幅・タブ側だけ縮む」構造
  （Flexibleの均等割り上限で日本語まで省略される罠を踏んだ→Expanded内に
  タブだけFlexibleで解決）。
- 実機TI10B1へ再配備済（端末ロックで自動起動のみ失敗・インストールは成功）。
- **入力画面への下タブ追加はユーザーが検討中→一旦見送り**（「でもいらないのかなぁ」）。
  実装するなら: カテゴリ表示2-3行化＋タブタップ時に未保存破棄の確認ダイアログ。
  代替案として「下書き保持」を抜本リデザイン時に検討するのが筋が良さそう。

### 追記6（2026-08-21 FB）

- **セルに収入も表示**: セルの金額は旧仕様「支出のみの略記マーカー」で収入が
  出なかった（FB「支出は27日のが反映されてるのに収入は25日のが反映されてない」）。
  dayIncomeTotalsProvider を追加し、支出=−赤／収入=+緑／予定=グレーの
  優先順・最大2行に（正方形セルの縦予算）。実機TI10B1へ配備・起動済。
