...
## 測定上の注意（最終横断レビューの指摘。golden set再校正パスで対処）

- vocab中のパーサキーワード衝突は全語彙で「時計」（homecenter）1語のみ。L2のキーワード落ち10%と重なった時だけ偽の合計候補になりうる＝現実的なトラップとして残置
- L1の日付なしレシートはシードの偶然で7/400（設計5%より薄い）。日付なし処理の検定力が低い点に留意。再校正時に層化サンプリング（20件に1件を決定的に日付なし）へ
- 真値日付がちょうど評価today（2026-07-11）のレシートは、日付行がノイズで壊れてもフォールバック（today返却）が正解扱いになる（≲1件/1,200規模）。再校正時に日付サンプル範囲からtodayを除外（`1 + rng.nextInt(364)`）
