...
- 実機 TI10B1 = **2.4.0(61)** 配備済み（今セッション分すべて入っている）。
- ブランチは **push 済み**（origin/feature/paper-design）。main マージは未。
...
ブロック）に統一した。選択中は枠を主色・太め＋文字色で示す。★**塗りは変えない**
（一度うっすら主色を敷いたら「選択してる場所は青背景にしないで。背景と交じって
分かりづらい」と差し戻された・2026-08-28）。
...
## ★次セッションの最初の仕事: シートのデザインを HTML で擦り合わせる

FB「計算方法と予算の設定を選ぶ場所はデザイン性が悪くなった。ここでPushまで
しちゃってまた次回セッションで**HTML作って擦り合わせて**いこう」。

- 対象は主に**歯車メニュー**（計算方法の変更／予算の設定の2択）。
  `SheetOptionCell` をそのまま流用したので、アイコン＋1行だけの選択肢が
  間延びして見える。計算方法シート（見出し＋説明つき）とは要件が違う。
- **実装を触る前に HTML のモックを作ってユーザーと合わせる**こと。
  いきなり Flutter を書き直さない。
- 決まっている制約: 選択中の塗りは変えない（枠と文字色だけ）／紙デザインの
  罫線色 kLine ／シートは showDragHandle: true。
