...
## ★次セッションの宿題（2026-08-28 ユーザー指示・未着手）

### 入力ページの「現金」ボタン

現状: `lib/features/entry/presentation/entry_screen.dart:338` あたりの `Wrap`
の中（「毎月の費用」「カテゴリを追加」と同じ行）にいる。キー `entry-payment-btn`。

要望:
1. **場所を変える** — 「金額と日付の間、日付の右隣」。
   ⚠️ 着手時に**どちらの意味か確認すること**（金額行と日付行の間に1行足すのか、
   日付の行の右端に置くのか）。今のところ後者＝日付行の右端と読んでいる。
2. **「現金」を開いたら「追加」ができる** — 支払い区分のシート
   （`_pickPaymentCard` / `entry_screen.dart:930` あたり）に「カードを追加」を
   足し、その場で登録できるようにする。いまは設定→カード管理まで行く必要がある。
3. カードを選ぶと**未払金**になる（現行どおり。仕様の確認）。
