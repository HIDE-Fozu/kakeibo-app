...
3. **分割払いの回数の選択肢を増やす（2026-08-18 ユーザー指定・未着手）**:
   現状は 2..36回（installment_page.dart の installment-count）。**48回やそれ以上が欲しい。
   主体はショッピングクレジットローンとして期間を増やし、住宅ローン基準も含める**。
   - 実装案（次回ユーザーと確認）: ドロップダウンを密→疎の段階刻みに
     （2..60回は1刻み → 66/72/84/96/108/120回 → 年単位 180/240/300/360/420回=35年）。
     420項目を全部並べるのはメニューが破綻するので刻みで間引くのが現実的。
   - 計算（installment_calc.dart）は任意 n で動くので変更不要。
     長期プランは一括起票の取引数が数百件になる（batch insert 化を検討）。
     編集時の開始月ドロップダウン（±18ヶ月）はそのままで良いか確認。
...
2. 今回10機能の実機受入FBの反映
3. 分割払いの発展: **回数の拡張（上記★3・48回〜住宅ローン級）**・固定費UIへの本統合・
   installmentCards のバックアップ同梱（DBテーブル化するなら v11）・支払い済み/残り回数の表示
