...
void main() {
  ...
  test('CSV has BOM, CRLF, header, and localized values', () async {
    ...
    expect(lines[0],
        'Date,Type,Amount,Category,Subcategory,Payment,Store,Memo,ReceiptID');
    // ヘッダ・区分・支払方法は英語固定。カテゴリ名/メモはユーザーデータなので日本語のまま。
    expect(lines[1], '2026-07-03,Expense,1200,食費,,E-money,,コンビニ,');
  });
  ...
  test('内訳の取引はカテゴリ列=親名・内訳列=自名になる', () async {
    ...
    final csv = await service.exportCsv();
    final lines = csv.substring(1).split('\r\n');
    expect(lines[1], '2026-07-05,Expense,980,食費,外食,,,,');
  });
  ...
  test('null paymentMethod and null memo render as empty fields', () async {
    ...
    final csv = await service.exportCsv();
    final lines = csv.substring(1).split('\r\n');
    expect(lines[1], '2026-07-25,Income,300000,食費,,,,,');
  });
}
